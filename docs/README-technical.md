...
**Release:** v1.0.0-rc1 (Round 18 — credential vault + HTTPS-always)

**Verification at this release (see `docs/SECURITY-AUDIT.md` for methodology):**

- pytest: 500/500 passing (tests/test_stress.py excluded from this gate; run nightly).
- Snyk Code: 0 HIGH / 0 MEDIUM findings.
- Snyk Open Source (SCA): 0 vulnerable paths across 45 dependencies.
- Bandit (`-ll`): 0 Medium / 0 High findings across 16 812 lines.
- pip-audit: 0 known vulnerabilities.
- Pentest suite: 32 adversarial tests (R14 + R17 + R18), all pass.
- Preflight gate: 0 blocking / 0 warnings with the documented production env.

---

## Round 18 Changes (what's new since v1.0.0-rc1 R17.3)

Round 18 hardens the credential surface and makes HTTPS non-optional. The changes are
deliberately narrow: no feature additions, no new dependencies, no API breakage for
callers that already used HTTPS.

### 1. Credential vault refactor (`CredentialEncryptor`)

Third-party credentials (Todoist, Notion, Gmail OAuth, LiteLLM proxy, Gemini,
Google refresh tokens) are no longer encrypted with the platform master key alone.
They are now encrypted under a key derived from **three independent secrets**:

$$\text{KEK} = \mathrm{HKDF\text{-}SHA256}\bigl(\text{ikm}=\text{pepper},\ \text{salt}=\mathrm{SHA256}(\text{tenant\_id}\ \Vert\ \text{0x1F}\ \Vert\ \text{owner\_google\_sub}),\ \text{info}=\text{"velaflow-credential-v2"},\ L=32\bigr)$$

- **pepper** — `VELAFLOW_CREDENTIAL_PEPPER`, a ≥32-byte base64 secret held only in the
  deployment's systemd `EnvironmentFile=`. Must differ from `VELAFLOW_MASTER_KEY`.
- **tenant_id** — binds ciphertext to one tenant; a row copied to another tenant is
  unreadable (even by the platform operator).
- **owner_google_sub** — the Google OAuth subject claim of the tenant's owner. It is
  written exactly once (`bind_owner_sub`) on the first successful OWNER/ADMIN OAuth
  login and is refused thereafter if a different sub is presented.

Encryption is AES-256-GCM. The **field name** is passed as authenticated associated data
(AAD), so a ciphertext encrypted as `todoist_api_token` cannot be relocated to
`gemini_api_key` even within the same tenant.

**Ciphertext layout:** `base64url(0x02 || nonce(12) || ct || tag)` — the schema byte
lets a future migration rotate the KDF without a big-bang backfill.

**Threat model explicitly covered:**

- Operator with root on the storage node has the ciphertext but not the pepper: cannot
  decrypt.
- Operator with pepper but not the per-tenant owner sub: cannot decrypt.
- Attacker who steals a database dump: needs both the pepper and every owner's sub.
- Ciphertext relocation (row-copy between tenants, field-copy within a tenant): rejected.
- Pepper rotation invalidates all existing credentials (intentional — forces re-entry).

The legacy `FieldEncryptor` (master-key + PBKDF2) is retained for **non-credential** data
at rest (e.g. the per-tenant `api_key` used for webhook signing), where the owner-sub
dimension would make self-service unusable.

### 2. HTTPS-only middleware (always-on)

`HTTPSOnlyMiddleware` now runs as the **outermost** middleware in every environment,
including local development. A plain-HTTP request receives `308 Permanent Redirect`
to the `https://` form of the same URL before any authentication, tenant resolution,
or CORS logic runs.

- `X-Forwarded-Proto: https` from a trusted reverse proxy (Caddy, nginx) is honoured —
  TLS termination upstream is still correct.
- The narrow loopback exemption (`127.0.0.1` / `::1` / `localhost`) applies **only** to
  the three health endpoints (`/health`, `/health/live`, `/health/ready`) and **only**
  when no `X-Forwarded-Proto` header is present, to avoid breaking systemd
  `ExecStartPost` probes that run before TLS material is available.

### 3. Preflight tightened (blocking env)

`scripts/preflight.py` now treats the following as **blocking** (exit 1 until set):

| Variable | Requirement |
|---|---|
| `VELAFLOW_MASTER_KEY` | base64, decodes to ≥32 bytes |
| `VELAFLOW_CREDENTIAL_PEPPER` | base64, decodes to ≥32 bytes, **must differ** from master key |
| `JWT_SECRET` | ≥32 characters |
| `GOOGLE_OAUTH_CLIENT_ID` | present |
| `GOOGLE_OAUTH_CLIENT_SECRET` | present |
| `VELAFLOW_TLS_CERT` | set, file exists, readable |
| `VELAFLOW_TLS_KEY` | set, file exists, readable |

The only informational-but-not-blocking checks left in preflight are `VELAFLOW_DATA_DIR`,
`VELAFLOW_LOG_HMAC_KEY`, and `VELAFLOW_DISABLE_OPEN_REGISTRATION` — three operator
choices with safe defaults.

### 4. Pentest suite (tests/test_pentest_r18.py)

Twenty new adversarial tests, grouped into three classes:

- `TestCredentialEncryptorPrimitives` (12 tests) — pepper validation, AAD binding,
  tenant / field / owner-sub relocation rejection, schema-version rejection,
  byte-flip authenticated-decryption failure, master-key isolation, pepper rotation.
- `TestOwnerSubBinding` (4 tests) — idempotent bind, refuse-rebind-to-different-sub,
  credentials refused before bind, decrypt refused when bind is missing.
- `TestHTTPSOnlyMiddleware` (3 tests) — cleartext protected routes are 308-redirected,
  `X-Forwarded-Proto: https` is honoured, loopback `/health` is exempt (verified via
  a hand-built ASGI scope because `TestClient` reports a non-loopback client host).
...
| 4 | **Telemetry an operator can audit** | `src/brain/security/action_ledger.py` writes an append-only, HMAC-chained JSON-Lines record of every user + system action into a path-sanitized directory, with an encrypted exporter. Tamper-evident: breaking the hash chain is detectable offline. |
...
| **Secure Structured Logging** | HMAC-chained tamper-evident logs with automatic PII/secret redaction (7 patterns), JSON structured output, log rotation, sanitised export for debugging |
...
tmpfs secrets, Caddy auto-HTTPS, and shareable sanitised logs.
...
## Round 19 Changes (what's new since R18)

### Zero Snyk findings across every severity

- `tests/_fakes.py` (new) returns runtime-random credential-shaped
  strings built via `secrets.token_hex` / `secrets.token_urlsafe`.
- Every fake literal token / password / API key in every test module
  was replaced with a call into `tests/_fakes.py`. Snyk Code now
  reports **0 findings at HIGH, MEDIUM and LOW**, no `.snyk` file,
  no per-line ignores.
...
|---|---|---|
| systemd unit | `LimitCORE=0` | Kernel refuses to write a core file; decrypted credentials in RAM cannot leak to `/var/lib/systemd/coredump`. |
| systemd unit | `LimitMEMLOCK=infinity` | The process can `mlockall(MCL_CURRENT \| MCL_FUTURE)`. |
| systemd unit | `LockPersonality=true` | Rejects attempts to switch execution personality (used by some ROP toolchains). |
| systemd unit | `RestrictRealtime=true` | Blocks real-time scheduling priority abuse. |
| systemd unit | `ProtectClock=true` | Read-only clock; prevents time-based key-reuse tricks. |
| systemd unit | `ProtectHostname=true` | Hostname is read-only. |
| Python runtime | `src/brain/security/memlock.py` | Calls `mlockall` and tightens `RLIMIT_CORE` to `(0, 0)` at API + worker startup. Silent no-op on non-Linux. |

All six `scripts/brain-*.service` files carry the full block. The
Python call is platform-guarded, so local development on Windows or
macOS is unaffected.

### End-to-end credential-in-transit proof

`tests/test_credential_in_transit.py` adds 14 scenarios that exercise
the full path a real third-party credential travels after R18:

1. `TenantManager.update_config` encrypts under `CredentialEncryptor`.
2. Only ciphertext is persisted on disk (verified by scanning every file
   under the tenant data directory for the plaintext substring).
3. `QueueWorker._build_tenant_settings` decrypts per-request into an
   ephemeral `Settings` instance.
4. The round-trip is covered for Todoist, Notion, Gmail, LiteLLM and
   Gemini.
5. Cross-tenant isolation: tenant A's ciphertext is rejected under
   tenant B's salt.
6. Tamper detection: rewriting `owner_google_sub` after the fact makes
...
| Signal | R18 | R19 |
|---|---|---|
| pytest | 500 passing | **514 passing** (+14) |
| Snyk Code | 25 LOW, 0 MED/HIGH | **0 across all severities** |
| Bandit | 0 M/H | 0 M/H |
| Snyk SCA | 0 | 0 |
| Preflight | 0 blocking / 0 warnings | 0 blocking / 0 warnings |
