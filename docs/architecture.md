...
- Every sink (`action_ledger._append`, `secure_logging._HMACRotatingHandler`,
  ...
  `scripts/preflight.py::_check_data_dir`)
...
graph LR
    subgraph SecureLogging["Secure Logging Pipeline"]
        ...
        EXPORT --> SHARED["Issue or support ticket<br/>Safe for debugging"]
    end
```
...
| **Sanitised Export** | `SecureLogger.export_sanitised()` | Double-redacts and produces Markdown that is safe to share for debugging |
...
flowchart TB
    ...
    Caddy --> API
    API --> Worker
    LogExport --> |"Sanitised logs"| ExportedLogs["/logs/export<br/>(local network only)"]
    ENV --> API
    ENV --> Worker
...
flowchart LR
    ...
    OCI -.-> |"SSH tunnel"| Exported["Exported logs<br/>/logs/export"]
