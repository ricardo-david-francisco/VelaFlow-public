...
| Logging | Sanitised export | Sanitised HMAC-chained logs every 6h |
| Updates | unattended-upgrades | Automatic Debian security patches |

### Exported log access

Sanitised logs are exported every 6 hours to `/opt/velaflow/data/exported-logs.md`.

- **Local network:** `GET http://<container-ip>/logs/export`
- **Via SSH tunnel:** `ssh -L 8080:<lxc-ip>:80 user@host` then `http://localhost:8080/logs/export`
