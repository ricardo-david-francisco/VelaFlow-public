...
| R5-4 | **HIGH** | Secret Exposure | Hardcoded personal e-mail address still in `config.py` and `docker-compose.yml` | Changed to empty string / required env var |
...
| R5-6 | **HIGH** | Secret Exposure | Personal Snyk organisation name in SECURITY-AUDIT.md | Redacted to `<redacted>` |
...
Snyk CLI v1.1304.0 was authenticated against a personal organisation and
...
- **Path traversal from env vars / CLI args** (Ã—14, all MEDIUM, priority
  ...
  `scripts/{drive_backup,preflight}.py` â€” **ACCEPTED
