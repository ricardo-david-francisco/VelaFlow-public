<!-- brand:header:start -->
<h1 align="center">VelaFlow</h1>

<p align="center">
  <b>Self-hosted, multi-tenant platform that turns tasks, calendar, email and notes<br>into one prioritised daily briefing.</b>
</p>
<p align="center">
  Per-tenant medallion pipeline on DuckDB, per-tenant credential encryption and optional LLM summaries.
</p>
...
<p align="center">
  ...
  <img alt="Ollama" src="https://img.shields.io/badge/Ollama-5A5A5A?logo=ollama&logoColor=white">
  <br>
  <img alt="Terraform" src="https://img.shields.io/badge/Terraform-844FBA?logo=terraform&logoColor=white">
  <img alt="Docker" src="https://img.shields.io/badge/Docker-2496ED?logo=docker&logoColor=white">
  <img alt="Kubernetes + KEDA" src="https://img.shields.io/badge/Kubernetes%20%2B%20KEDA-326CE5?logo=kubernetes&logoColor=white">
  ...
  <br>
  <img alt="528 tests passing" src="https://img.shields.io/badge/tests-528%20passing-brightgreen">
  <img alt="Snyk Code 0 issues" src="https://img.shields.io/badge/Snyk%20Code-0%20issues-brightgreen?logo=snyk&logoColor=white">
  <img alt="licence MIT" src="https://img.shields.io/badge/licence-MIT-blue">
</p>

<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="docs/assets/velaflow-architecture-dark.svg">
    <img alt="VelaFlow architecture: five sources feed a per-tenant Bronze, Silver and Gold pipeline on DuckDB with an optional LLM layer; results go out by email, Notion, WhatsApp, REST API and a web GUI." src="docs/assets/velaflow-architecture-light.svg" width="100%">
  </picture>
</p>
...
<table>
  <tr><td width="190"><b>What it does</b></td><td>Collects tasks, events, email and notes from Todoist, Notion, Google Calendar and Gmail, ranks them together and sends a daily briefing by email, Notion or WhatsApp.</td></tr>
  <tr><td width="190"><b>How it works</b></td><td>Each tenant has its own Bronze → Silver → Gold pipeline on DuckDB. A deterministic model ranks the items, and an LLM chain (Gemini, Groq, local Ollama) writes the summary when one is available.</td></tr>
  <tr><td width="190"><b>Security</b></td><td>Third-party tokens are encrypted with AES-256-GCM under a key that needs both the operator pepper and the tenant's Google identity, so shell access to the host is not enough to read them.</td></tr>
  <tr><td width="190"><b>Deployment</b></td><td>A Terraform module targets Proxmox LXC, any Linux VM and Oracle Cloud Always Free. Kubernetes and KEDA manifests cover scaling out.</td></tr>
</table>

<table>
  <tr>
    <td align="center"><b>15,000+</b><br>lines of Python</td>
    <td align="center"><b>528</b><br>tests passing</td>
    <td align="center"><b>0</b><br>medium+ findings<br>(Snyk, Bandit, pip-audit)</td>
    <td align="center"><b>58</b><br>security test cases<br>over 19 audit rounds</td>
    <td align="center"><b>€0</b><br>monthly hosting</td>
  </tr>
</table>
...
| Area | Used for |
|---|---|
| **Data** | DuckDB medallion pipeline (Bronze / Silver / Gold), tenant partitioning, schema validation, deduplication, PII masking, SQLite catalog |
| **API and UI** | FastAPI, Google OAuth + JWT, role-based access, server-side quotas, Stripe webhooks, Streamlit |
| **LLM** | Gemini → Groq → Ollama fallback chain, local inference, RAG on DuckDB vector search, versioned prompt templates |
| **Security** | AES-256-GCM with HKDF-derived per-tenant keys, HMAC-chained audit log, HTTPS-only middleware, systemd sandboxing, mlockall |
| **Infrastructure** | Terraform, Docker / Podman, Kubernetes with KEDA and HPA, Prometheus, Grafana, preflight checks before start-up |
| **Docs** | Architecture decision records in <a href="docs/adr">docs/adr</a>, and a comparison with managed cloud services in <a href="#3-familiar-stack-translation">Familiar-stack translation</a> |
...
- [1. Vision](#1-vision)
- [2. Why VelaFlow exists](#2-why-velaflow-exists)
...
- [13. Codebase size](#13-codebase-size)
...
The product is offered in tiers:
...
## 2. Why VelaFlow exists

### 2.1 The problem for users
...
### 2.4 Failure modes this design avoids

Public incidents are named where they apply.
...
### 2.5 Security controls in detail
...
| Mosaic AI / Vertex AI / Bedrock (managed LLM orchestration, RAG) | `src/brain/llm.py` + `src/brain/llm_local.py` multi-model fallback + `src/brain/rag.py` (chunker, embedder, DuckDB-backed vector store) | Avoids vendor lock-in; every tenant pays for only the upstream LLM minutes they use; Premium/VIP tiers can run fully local with Ollama. |
...
| Kubernetes + KEDA + HPA (event-driven autoscaling) | Same — `deploy/kubernetes/` includes KEDA `ScaledObject` manifests for the worker; HPA for the API; GPU pool on a separate node-selector | Identical pattern, same manifests. The LXC path is simply "scale-to-one". |
...
| Terraform / Pulumi / Helm | `deploy/terraform/` — one shared host module and three targets (Proxmox LXC, any Linux VM, Oracle Cloud Always Free); `deploy/kubernetes/` manifests for scale-out; Helm charts are a later packaging step | Portable, declarative host IaC keeps hosting at €0 today; the same module moves to a paid cloud only when revenue justifies it (see [Deployment](#10-deployment)). |
...
| SAP / S/4HANA / BW/4HANA extractors | Out of scope. VelaFlow ingests from Todoist, Notion, Google Calendar, Gmail, and user-uploaded documents | Each source is its own small module (`src/brain/todoist.py`, `notion.py`, `gmail.py`, `calendar_ctx.py`), so an additional source is a new connector, not a platform rewrite. |

In short, the **shape** of each
...
Rows marked *out of scope* are listed for completeness.
...
│  ├─ rag.py                  Chunker, embedder, vector store
...
Written and maintained by one person since **2026-04-17**. Measured at the current version:
